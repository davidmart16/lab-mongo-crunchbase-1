      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "5":
            ...
            db.collection('companies').find( {$and: [{'founded_year': 2004} , {$and: [{ founded_month: { $gte: 4 } }, { founded_month: { $lte: 6 } }]} ]}, {name: 1, _id: 0 } ).sort({created_at: 1}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "11":
            ...
            db.collection('companies').find({$and:[{'name': 'Facebook'}, {'relationships.is_past': false}]}, {'relationships.person.first_name': 1, 'relationships.person.last_name': 1, _id: 0}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "15":
            // 15.- How many companies that has "social-network" in tag-list and founded between 2002 and 2016 inclusive
             db.collection('companies').find({$and:[{'tag_list': {$in:['social-networking']}}, {$and:[{'founded_year': {$gte: 2002} } , {'founded_year': {$lte: 2016} }]}]}, {name: 1, _id: 0}).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          case "17":
            ...
            db.collection('companies').find({$and:[{'tag_list': {$in:['social-networking']}}, {$and:[{'founded_year': {$gte: 2002} } , {'founded_year': {$lte: 2016} }]}, {'offices.city': 'New York'}]}, {name: 1, _id: 0}).count((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
